...
./configure \
  ...
  --enable-filter='buffersrc,buffersink,format,null,firequalizer,realtime,latency,deinterlace_v4l2m2m,scale_v4l2m2m,format,scale,fps,setdar,settb,setpts,asettb,asetpts,concat,aresample,aformat,volume,loudnorm,equalizer,bass,treble,crystalizer,ladspa,lv2,rubberband,afade' \
...
./configure \
 ...
 --disable-indev=pulse --disable-outdev=pulse && make -j2 && sudo make install   --disable-libx264 --disable-libx265 --disable-omx --disable-omx-rpi --disable-mmal --disable-amf --disable-vdpau --disable-vaapi \
make -j2
sudo make install
