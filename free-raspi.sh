https://github.com/spookyfirehorse/raspberry-pi-with-ffmpeg-all-codecs-mpv-rpi-.git && \
...
make -j4 && \
sudo make install && sudo ldconfig && sudo apt build-dep ffmpeg && mkdir -p ~/bin && \
...
PATH="$HOME/bin:$PATH" make -j 4 && make install && cd ~/bin && sudo cp ff* /usr/local/bin/ && sudo apt build-dep mpv && \
