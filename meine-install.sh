...
# Abhängigkeiten installieren (modernisierte Header-Suche)
apt update
KERNEL_VERSION=$(uname -r)
HEADER_PKG="linux-headers-$KERNEL_VERSION"
if ! apt-cache show "$HEADER_PKG" > /dev/null 2>&1; then
    HEADER_PKG="linux-headers-rpi-$(uname -m | sed 's/armv7l/v7l/;s/aarch64/v8/')"
fi

apt-get -y install "$HEADER_PKG" dkms git i2c-tools libasound2-plugins
...
# Wir kopieren sie unter ihrem neuen Namen in das Overlay-Verzeichnis
...
# config.txt anpassen (Referenz auf den finalen Fix)
...
# Alte Einträge entfernen, um Konflikte zu vermeiden
sed -i '/dtoverlay=wm8960-soundcard/d' $CONFIG_PATH

# Neuen Fix eintragen (ohne .dtbo Endung in der config.txt)
