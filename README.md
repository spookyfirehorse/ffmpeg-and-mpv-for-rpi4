...
#    running with  imx 708, but it works with all cameras. the only differents is with hight and autofocus full comand
         ...
         --low-latency 1 --framerate 24  -b 1000000  --codec libav --libav-format flv   --profile=main --level 4.1 \
         ...
         -c:v  h264_v4l2m2m   -b:v 1M  -maxrate 1M -minrate 1M  -bufsize 2000k -fps_mode:v cfr -filter:v  fps=fps=source_fps:round=near \
         -threads $(nproc)  -c:a libfdk_aac -profile:a aac_he  -b:a 32k -vbr 0  -max_muxing_queue_size 9999 \
         -flush_packets 0  -bf 48  -f rtsp -rtsp_transport udp rtsp://localhost:8554/mystream
  ...
  # winner of all
               ...
               -hwaccel drm -hwaccel_output_format drm_prime -i -  -metadata title='Devil'  -codec copy -copyts      \
